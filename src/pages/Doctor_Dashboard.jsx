...
export default function DoctorDashboard() {
  ...
  const handleNext = async () => {
    ...
    // Just highlight in UI (handled in render)
  ...
  /* ==================== COMPLETE CONSULTATION (save + mark Completed) ==================== */
  const handleComplete = async () => {
    ...
    try {
      ...
      const freshTokens = await getAllTockens();
      const freshHistory = await getAllPatientHistory();
      if (Array.isArray(freshTokens) && Array.isArray(freshHistory)) {
        const filteredTokens = freshTokens.filter(
          (t) => t.doctorId == doctorId || t.doctorName === doctorName
        );
        const filteredHistory = freshHistory.filter(
          (h) => h.doctorId == doctorId || h.doctorName === doctorName
        );
        setQueue(filteredTokens);
        setPatientHistory(filteredHistory);
      }

      alert("Prescription saved and consultation completed!");
    } catch (e) {
      console.error(e);
      alert("Failed to save prescription or update status.");
    } finally {
      setActionLoading(false);
    }
  ...
  const handleSkip = async () => {
    ...
    try {
      ...
      // Refresh queue
      const fresh = await getAllTockens();
        ...
        <main className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-lg text-gray-700">
                  Manage consultations, prescriptions, and appointments.
                </p>
              </div>
              <button
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-1">Tokens Consultations</p>
            </motion.div>
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-1">Tokens Patients</p>
            </motion.div>
          </motion.section>
            ...
            >
              ...
              <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                {queue.length === 0 ? (
                  ...
                  // Sort queue by tokenNumber (ascending)
                  [...queue]
                    ...
                    .map((p, i) => (
                      <motion.div
                        ...
                        className={`cursor-pointer p-4 rounded-xl border-2 transition-all hover:scale-[1.02] hover:shadow-md ${currentPatient?.id === p.id
                  ...
                  appointments.map((a) => (
                    ...
                    >
                      ...
                      <div className="mt-3 flex justify-between items-center">
                        <span
                          className={`text-xs font-semibold px-3 py-1 rounded-full ${a.status === "Completed"
                        ...
                        <button
                          ...
                          className={`text-xs px-3 py-1 rounded-lg transition-all font-medium ${a.status === "Completed"
